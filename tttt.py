...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.img_item.scene().sigMouseClicked.connect(self.mouse_clicked)
    ...
    def update_image(self):
        # Wyświetlanie gridu:
        self.img_item.setImage(self.grid.T)  # lub bez .T, zależnie od potrzeb

        # Ustawienie zakresów osi:
        self.plot_widget.setLimits(
            xMin=self.xi[0], xMax=self.xi[-1],
            yMin=self.yi[0], yMax=self.yi[-1]
        )
        self.plot_widget.setAspectLocked(False)

        # Ustawienie ticków, jeśli chcesz:
        self.plot_widget.getAxis('bottom').setLabel('X (mm)')
        self.plot_widget.getAxis('left').setLabel('Y (mm)')
        # self.image_view.setImage(self.grid, autoLevels=True)  # .T for correct orientation
        self.seed_points = []
